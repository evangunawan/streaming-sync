...
class API {
  ...
  async authenticate() {
    const params = {
      ...
      scope:
        'user-read-email user-read-private playlist-read-collaborative playlist-read-private user-library-read',
    };
  ...
  async getAccountInfo() {
    ...
    let userData;
    try {
      userData = await this.apiClient.reqGet('https://api.spotify.com/v1/me', {
  ...
  async getUserPlaylist(account_id) {
    ...
    try {
      result = await this.apiClient.reqGet(
        `https://api.spotify.com/v1/users/${account_id}/playlists`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
